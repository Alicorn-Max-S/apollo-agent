# ============================================================================
# Apollo Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$ApolloHome = "$env:LOCALAPPDATA\apollo",
    [string]$InstallDir = "$env:LOCALAPPDATA\apollo\apollo-agent"
)
...
function Test-Node {
    ...
    $managedNode = "$ApolloHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$ApolloHome\node;$env:Path"
        Write-Success "Node.js $version found (Apollo-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.apollo/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\apollo-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ApolloHome\node") { Remove-Item -Recurse -Force "$ApolloHome\node" }
                Move-Item $extractedDir.FullName "$ApolloHome\node"
                $env:Path = "$ApolloHome\node;$env:Path"

                $version = & "$ApolloHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.apollo/node/"
                $script:HasNode = $true
...
function Set-PathVariable {
    Write-Info "Setting up apollo command..."
    
    if ($NoVenv) {
        $apolloBin = "$InstallDir"
    } else {
        $apolloBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so apollo is globally available
    # On Windows, the apollo.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$apolloBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$apolloBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $apolloBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set APOLLO_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\apollo instead
    # of the Unix default ~/.apollo
    $currentApolloHome = [Environment]::GetEnvironmentVariable("APOLLO_HOME", "User")
    if (-not $currentApolloHome -or $currentApolloHome -ne $ApolloHome) {
        [Environment]::SetEnvironmentVariable("APOLLO_HOME", $ApolloHome, "User")
        Write-Success "Set APOLLO_HOME=$ApolloHome"
    }
    $env:APOLLO_HOME = $ApolloHome
    
    # Update current session
    $env:Path = "$apolloBin;$env:Path"
    
    Write-Success "apollo command ready"
}
...
function Copy-ConfigTemplates {
    ...
    New-Item -ItemType Directory -Force -Path "$ApolloHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ApolloHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ApolloHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ApolloHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ApolloHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ApolloHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ApolloHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ApolloHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ApolloHome\skills" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ApolloHome\whatsapp\session" | Out-Null
    
    # Create .env
    $envPath = "$ApolloHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.apollo/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.apollo/.env"
        }
    } else {
        Write-Info "~/.apollo/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$ApolloHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.apollo/config.yaml from template"
        }
    } else {
        Write-Info "~/.apollo/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$ApolloHome\SOUL.md"
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.apollo/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.apollo/"
    
    # Seed bundled skills into ~/.apollo/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.apollo/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.apollo/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ApolloHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.apollo/skills/"
            }
        }
    }
}
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m apollo_cli.main setup
    } else {
        python -m apollo_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $apolloCmd = "$InstallDir\venv\Scripts\apollo.exe"
    if (-not (Test-Path $apolloCmd)) {
        $apolloCmd = "apollo"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ApolloHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'apollo whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ApolloHome\logs\gateway.log"
            Start-Process -FilePath $apolloCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ApolloHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: apollo gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: apollo gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ApolloHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ApolloHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ApolloHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ApolloHome\apollo-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   apollo              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   apollo setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   apollo config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   apollo config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   apollo gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   apollo update       " -NoNewline -ForegroundColor Green
