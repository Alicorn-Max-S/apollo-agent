...
VERSION_FILE = REPO_ROOT / "apollo_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "apollo@nousresearch.com": "NousResearch",
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/Alicorn-Max-S/apollo-agent",
    ...
    lines.append(f"# Apollo Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Apollo Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Apollo Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Apollo Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        git("tag", "-a", tag_name, "-m",
            f"Apollo Agent v{new_version} ({calver_date})\n\nWeekly release")
        ...
        result = subprocess.run(
            ["gh", "release", "create", tag_name,
             "--title", f"Apollo Agent v{new_version} ({calver_date})",
        ...
        if result.returncode == 0:
            ...
            print(f"    gh release create {tag_name} --title 'Apollo Agent v{new_version} ({calver_date})'")
