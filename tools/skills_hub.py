#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Apollo Skills Hub.
...
Used by apollo_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            apollo_meta = metadata.get("apollo", {})
            if isinstance(apollo_meta, dict):
                tags = apollo_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Alicorn-Max-S) but not activated
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                apollo_meta = meta_block.get("apollo", {})
                if isinstance(apollo_meta, dict):
                    tags = apollo_meta.get("tags", [])
