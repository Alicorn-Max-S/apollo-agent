...
import yaml
from apollo_cli.config import load_env, _ENV_VAR_NAME_RE
...
# All skills live in ~/.apollo/skills/ (seeded from bundled skills/ on install).
...
def _is_gateway_surface() -> bool:
    if os.getenv("APOLLO_GATEWAY_SESSION"):
        return True
    return bool(os.getenv("APOLLO_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return "Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to ~/.apollo/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.apollo/skills/mlops/axolotl/SKILL.md -> "mlops"
    """
    try:
...
def _get_disabled_skill_names() -> Set[str]:
    ...
    Resolves platform from ``APOLLO_PLATFORM`` env var, falls back to
    ...
    try:
        ...
        resolved_platform = os.getenv("APOLLO_PLATFORM")
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    try:
        ...
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("APOLLO_PLATFORM")
        if resolved_platform:
            platform_disabled = skills_cfg.get("platform_disabled", {}).get(resolved_platform)
            if platform_disabled is not None:
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.apollo/skills/.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``apollo skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": "No skills found. Skills directory created at ~/.apollo/skills/",
                },
                ensure_ascii=False,
            )
...
def skill_view(name: str, file_path: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        # Check metadata.apollo.* first (agentskills.io convention), fall back to top-level
        apollo_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            apollo_meta = metadata.get("apollo", {}) or {}

        tags = _parse_tags(apollo_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            apollo_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
