"""Apollo CLI skin/theme engine.

A data-driven skin system that lets users customize the CLI's visual appearance.
Skins are defined as YAML files in ~/.apollo/skins/ or as built-in presets.
...
- ``default`` — Classic Apollo gold/kawaii (the current look)
...
Drop a YAML file in ``~/.apollo/skins/<name>.yaml`` following the schema above.
...
def _skins_dir() -> Path:
    ...
    home = Path(os.getenv("APOLLO_HOME", Path.home() / ".apollo"))
    return home / "skins"
