#!/usr/bin/env python3
"""
Apollo CLI - Main entry point.

Usage:
    apollo                     # Interactive chat (default)
    apollo chat                # Interactive chat
    apollo gateway             # Run gateway in foreground
    apollo gateway start       # Start gateway as service
    apollo gateway stop        # Stop gateway service
    apollo gateway status      # Show gateway status
    apollo gateway install     # Install gateway service
    apollo gateway uninstall   # Uninstall gateway service
    apollo setup               # Interactive setup wizard
    apollo logout              # Clear stored authentication
    apollo status              # Show status of all components
    apollo cron                # Manage cron jobs
    apollo cron list           # List cron jobs
    apollo cron status         # Check if cron scheduler is running
    apollo doctor              # Check configuration and dependencies
    apollo honcho setup                    # Configure Honcho AI memory integration
    apollo honcho status                   # Show Honcho config and connection status
    apollo honcho sessions                 # List directory → session name mappings
    apollo honcho map <name>               # Map current directory to a session name
    apollo honcho peer                     # Show peer names and dialectic settings
    apollo honcho peer --user NAME         # Set user peer name
    apollo honcho peer --ai NAME           # Set AI peer name
    apollo honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    apollo honcho mode                     # Show current memory mode
    apollo honcho mode [hybrid|honcho|local]  # Set memory mode
    apollo honcho tokens                   # Show token budget settings
    apollo honcho tokens --context N       # Set session.context() token cap
    apollo honcho tokens --dialectic N     # Set dialectic result char cap
    apollo honcho identity                 # Show AI peer identity representation
    apollo honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    apollo honcho migrate                  # Step-by-step migration guide: OpenClaw native → Apollo + Honcho
    apollo version             Show version
    apollo update              Update to latest version
    apollo uninstall           Uninstall Apollo Agent
    apollo acp                 Run as an ACP server for editor integration
    apollo sessions browse     Interactive session picker with search

    apollo claw migrate --dry-run  # Preview migration without changes
"""
...
# Load .env from ~/.apollo/.env first, then project root as dev fallback
from dotenv import load_dotenv
from apollo_cli.config import get_env_path, get_apollo_home
...
# Point mini-swe-agent at ~/.apollo/ so it shares our config
os.environ.setdefault("MSWEA_GLOBAL_CONFIG_DIR", str(get_apollo_home()))
...
from apollo_cli import __version__, __release_date__
from apollo_constants import OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from apollo_cli.config import get_env_path, get_apollo_home
    from apollo_cli.auth import get_auth_status
    ...
    auth_file = get_apollo_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _resolve_last_cli_session() -> Optional[str]:
    ...
    try:
        from apollo_state import SessionDB
        db = SessionDB()
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'apollo sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Apollo isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  apollo setup")
        print()

        from apollo_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'apollo setup' at any time to configure.")
        sys.exit(1)
...
def cmd_gateway(args):
    ...
    from apollo_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Apollo?")
        print()
    ...
    session_dir = get_apollo_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: apollo gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  apollo gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with 'Apollo Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  apollo gateway")
        ...
        print()
        print("  Or install as a service: apollo gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'apollo whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from apollo_cli.setup import run_setup_wizard
    run_setup_wizard(args)


def cmd_model(args):
    """Select default model — starts with provider selection, then model picker."""
    from apollo_cli.auth import (
        ...
        _login_nous, _get_recent_models,
    )
    from apollo_cli.config import load_config, save_config, get_env_value, save_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("APOLLO_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
    ...
    # Build combined menu: "Previously Used" models at top, then providers
    recent_global = _get_recent_models(limit=3)
    combined = []
    if recent_global:
        combined.append(("__sep_recent__", "── Previously Used ──"))
        for mid in recent_global:
            lbl = f"{mid}  ← currently in use" if mid == current_model else mid
            combined.append((f"__recent:{mid}", lbl))
        combined.append(("__sep_providers__", "── Providers ──"))
    for key, label in ordered:
        combined.append((key, label))

    provider_idx = _prompt_provider_choice([label for _, label in combined])
    if provider_idx is None:
        print("No change.")
        return

    selected_key = combined[provider_idx][0]
    ...
    if selected_key.startswith("__recent:"):
        ...
        # Resolve and update the provider for this model so credentials work
        from apollo_cli.models import detect_provider_for_model
        detected = detect_provider_for_model(model_id)
        if detected:
            from apollo_cli.auth import deactivate_provider
            cfg = load_config()
            model_cfg = cfg.get("model")
            if not isinstance(model_cfg, dict):
                model_cfg = {"default": model_id}
                cfg["model"] = model_cfg
            model_cfg["provider"] = detected
            # Set base_url for providers that need it
            if detected == "openrouter":
                model_cfg["base_url"] = OPENROUTER_BASE_URL
            elif detected in ("anthropic",):
                model_cfg.pop("base_url", None)  # Anthropic resolves its own URL
            save_config(cfg)
            deactivate_provider()
    ...
    # Handle separator or cancel selection
    if selected_key.startswith("__sep_") or selected_key == "cancel":
        print("No change.")
        return
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from apollo_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider, _get_recent_models_split
    from apollo_cli.config import get_env_value, save_env_value
    ...
    from apollo_cli.models import model_ids
    openrouter_models = model_ids()

    provider_recent, global_recent = _get_recent_models_split(openrouter_models, limit=5)
    selected = _prompt_model_selection(openrouter_models, current_model=current_model,
                                       provider_recent=provider_recent, global_recent=global_recent,
    ...
    if selected:
        ...
        from apollo_cli.config import load_config, save_config
        cfg = load_config()
        model = cfg.get("model")
...
def _model_flow_nous(config, current_model=""):
    ...
    from apollo_cli.auth import (
        ...
        _login_nous, PROVIDER_REGISTRY, _get_recent_models_split,
    )
    from apollo_cli.config import get_env_value, save_env_value
    import argparse
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from apollo_cli.auth import (
        ...
        PROVIDER_REGISTRY, DEFAULT_CODEX_BASE_URL, _get_recent_models_split,
    )
    from apollo_cli.codex_models import get_codex_model_ids
    from apollo_cli.config import get_env_value, save_env_value
    import argparse
    ...
    _codex_token = None
    try:
        from apollo_cli.auth import resolve_codex_runtime_credentials
    ...
    provider_recent, global_recent = _get_recent_models_split(codex_models, limit=5)
...
def _model_flow_custom(config):
    ...
    """
    from apollo_cli.auth import _save_model_choice, deactivate_provider
    from apollo_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    if model_name:
        ...
        if base_url or api_key:
            deactivate_provider()
        print("Endpoint saved. Use `/model` in chat or `apollo model` to set a model.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from apollo_cli.auth import _save_model_choice, deactivate_provider
    from apollo_cli.config import save_env_value, load_config, save_config
    from apollo_cli.models import fetch_api_models
...
def _model_flow_kimi(config, current_model=""):
    ...
    from apollo_cli.auth import (
        ...
        _save_model_choice, deactivate_provider, _get_recent_models_split,
    )
    from apollo_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    if model_list:
        provider_recent, global_recent = _get_recent_models_split(model_list, limit=5)
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from apollo_cli.auth import (
        ...
        _update_config_for_provider, deactivate_provider, _get_recent_models_split,
    )
    from apollo_cli.config import get_env_value, save_env_value, load_config, save_config
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    from apollo_cli.config import save_anthropic_oauth_token
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from apollo_cli.models import _PROVIDER_MODELS
    ...
    if selected:
        ...
        # providers if the user switches without running 'apollo model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Apollo CLI with a provider."""
    from apollo_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from apollo_cli.auth import logout_command
    logout_command(args)


def cmd_status(args):
    """Show status of all components."""
    from apollo_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from apollo_cli.cron import cron_command
    cron_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from apollo_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from apollo_cli.config import config_command
    config_command(args)


def cmd_version(args):
    """Show version."""
    print(f"Apollo Agent v{__version__} ({__release_date__})")
...
def cmd_uninstall(args):
    """Uninstall Apollo Agent."""
    from apollo_cli.uninstall import run_uninstall
    run_uninstall(args)


def _update_via_zip(args):
    """Update Apollo Agent by downloading a ZIP archive.
    ...
    try:
        tmp_dir = tempfile.mkdtemp(prefix="apollo-update-")
        zip_path = os.path.join(tmp_dir, f"apollo-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to apollo-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"apollo-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("apollo-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Apollo behaves unexpectedly.")
    ...
    print("  Review `git diff` / `git status` if Apollo behaves unexpectedly.")
    return True
    ...
    try:
        ...
        if needs_migration:
            ...
            if response in ('', 'y', 'yes'):
                ...
                print()
                print("Skipped. Run 'apollo config migrate' later to configure.")
        else:
        ...
        try:
            check = subprocess.run(
                ["systemctl", "--user", "is-active", "apollo-gateway"],
            ...
            if check.stdout.strip() == "active":
                ...
                restart = subprocess.run(
                    ["systemctl", "--user", "restart", "apollo-gateway"],
                ...
                if restart.returncode == 0:
                    ...
                    print("  Try manually: apollo gateway restart")
        except (FileNotFoundError, subprocess.TimeoutExpired):
            pass  # No systemd (macOS, WSL1, etc.) — skip silently
        
        print()
        print("Tip: You can now select a provider and model:")
        print("  apollo model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``apollo -c Pokemon Agent Dev`` without quoting the
...
def main():
    """Main entry point for apollo CLI."""
    parser = argparse.ArgumentParser(
        prog="apollo",
        description="Apollo Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    apollo                        Start interactive chat
    apollo chat -q "Hello"        Single query mode
    apollo -c                     Resume the most recent session
    apollo -c "my project"        Resume a session by name (latest in lineage)
    apollo --resume <session_id>  Resume a specific session by ID
    apollo setup                  Run setup wizard
    apollo logout                 Clear stored authentication
    apollo model                  Select default model
    apollo config                 View configuration
    apollo config edit            Edit config in $EDITOR
    apollo config set model gpt-4 Set a config value
    apollo gateway                Run messaging gateway
    apollo -w                     Start in isolated git worktree
    apollo gateway install        Install as system service
    apollo sessions list          List past sessions
    apollo sessions browse        Interactive session picker
    apollo sessions rename ID T   Rename/title a session
    apollo update                 Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Apollo Agent"
    )
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Apollo Agent with an interactive wizard. "
                    "Run a specific section: apollo setup model|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Apollo CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: apollo-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Apollo Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Apollo Agent setup"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Apollo Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from apollo_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            from apollo_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from apollo_cli.skills_hub import skills_command
            skills_command(args)
    ...
    honcho_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "each turn so Apollo knows who you are without you having to repeat yourself.\n\n"
    ...
    honcho_subparsers.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Apollo Honcho",
    )
    ...
    def cmd_tools(args):
        from apollo_cli.tools_config import tools_command
        tools_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch apollo --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            apollo_bin = shutil.which("apollo")
            if apollo_bin:
                os.execvp(apollo_bin, ["apollo", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "apollo_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Apollo"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Apollo",
    ...
    def cmd_claw(args):
        from apollo_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Apollo Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Apollo Agent",
        description="Remove Apollo Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Apollo Agent as an ACP (Agent Client Protocol) server",
        description="Start Apollo Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Apollo Agent as an ACP server."""
        try:
    ...
    # e.g. ``apollo -c Pokemon Agent Dev`` → ``apollo -c 'Pokemon Agent Dev'``
