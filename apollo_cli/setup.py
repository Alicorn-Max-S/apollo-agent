"""
Interactive setup wizard for Apollo Agent.
...
Config files are stored in ~/.apollo/ for easy access.
"""
...
def _setup_provider_model_selection(config, provider_id, current_model, prompt_choice, prompt_fn):
    ...
    from apollo_cli.config import get_env_value
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("Apollo Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Apollo using environment variables or config commands:")
    print_info("  apollo config set model.provider custom")
    print_info("  apollo config set model.base_url http://localhost:8080/v1")
    print_info("  apollo config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'apollo setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning(f"  Skipped (configure later with 'apollo setup')")


def _print_setup_summary(config: dict, apollo_home):
    ...
    if get_env_value("OPENROUTER_API_KEY"):
        ...
        try:
            _vauth_path = Path(os.environ.get("APOLLO_HOME", Path.home() / ".apollo")) / "auth.json"
    ...
    if _has_vision:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'apollo setup' to configure"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'apollo setup tools' to configure them,"
        )
    ...
    print(color("📁 All your files are in ~/.apollo/:", Colors.CYAN, Colors.BOLD))
    print()
    print(f"   {color('Settings:', Colors.YELLOW)}  {get_config_path()}")
    print(f"   {color('API Keys:', Colors.YELLOW)}  {get_env_path()}")
    print(
        f"   {color('Data:', Colors.YELLOW)}      {apollo_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('apollo setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('apollo setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('apollo setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('apollo setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('apollo setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('apollo config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('apollo config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('apollo config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('apollo', Colors.GREEN)}              Start chatting")
    print(f"   {color('apollo gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('apollo doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `apollo tools` and `apollo setup tools`).
...
def setup_model_provider(config: dict):
    ...
    if not has_any_provider:
        print_warning("An inference provider is required for Apollo to work.")
        print()
    ...
    if provider_idx == 0:  # Nous Portal (OAuth)
        ...
        try:
            from apollo_cli.auth import _login_nous, ProviderConfig
            import argparse
            ...
            print_info("You can try again later with: apollo model")
        ...
        config_path = (
            Path(os.environ.get("APOLLO_HOME", Path.home() / ".apollo")) / "config.yaml"
        )
        try:
            disk_cfg = {}
        ...
        if base_url:
            ...
            config_path = (
                Path(os.environ.get("APOLLO_HOME", Path.home() / ".apollo"))
                / "config.yaml"
            )
            try:
                disk_cfg = {}
        ...
        if api_key:
            ...
            from apollo_cli.auth import detect_zai_endpoint
        ...
        from apollo_cli.auth import PROVIDER_REGISTRY
        from apollo_cli.config import save_anthropic_api_key, save_anthropic_oauth_token
    ...
    if _vision_needs_setup:
        if get_env_value("OPENROUTER_API_KEY"):
            ...
            try:
                _auth_path = Path(os.environ.get("APOLLO_HOME", Path.home() / ".apollo")) / "auth.json"
    ...
    if _vision_needs_setup:
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'apollo config set OPENROUTER_API_KEY ...'")
    ...
    if selected_provider != "custom":  # Custom already prompted for model name
        ...
        if selected_provider == "nous" and nous_models:
            ...
            from apollo_cli.models import provider_model_ids
            ...
            from apollo_cli.models import model_ids, menu_labels
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Apollo runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Apollo via Telegram/Discord, this is where")
        print_info(
...
def setup_agent_settings(config: dict):
    ...
    current_max = get_env_value("APOLLO_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("APOLLO_MAX_ITERATIONS", str(max_iter))
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Apollo from anywhere.")
    print()
    ...
    if not existing_telegram and prompt_yes_no("Set up Telegram bot?", False):
        ...
        if token:
            ...
            print()
            print_info("📬 Home Channel: where Apollo delivers cron job results,")
    ...
    if not existing_slack and prompt_yes_no("Set up Slack bot?", False):
        ...
        print()
        print_info(
            "   Full guide: https://apollo-agent.ai/docs/user-guide/messaging/slack"
        )
        print()
    ...
    if not existing_whatsapp and prompt_yes_no("Set up WhatsApp?", False):
        ...
        print_info("Requires Node.js. Run 'apollo whatsapp' for guided setup.")
        print()
        if prompt_yes_no("Enable WhatsApp now?", True):
            save_env_value("WHATSAPP_ENABLED", "true")
            print_success("WhatsApp enabled")
            print_info("Run 'apollo whatsapp' to choose your mode (separate bot number")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     apollo config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        from apollo_cli.gateway import (
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: apollo gateway install")
            else:
                print_info("  You can install later: apollo gateway install")
                print_info("  Or run in foreground:  apollo gateway")
        else:
            print_info("Start the gateway to bring your bots online:")
            print_info("   apollo gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `apollo setup tools` and `apollo tools` use the same flow:
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_apollo.py"
)


def _offer_openclaw_migration(apollo_home: Path) -> bool:
    ...
    print_info("Apollo can import your settings, memories, skills, and API keys.")
    print()
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "openclaw_to_apollo", _OPENCLAW_SCRIPT
        )
        ...
        migrator = mod.Migrator(
            ...
            target_root=apollo_home.resolve(),
            execute=True,
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Apollo.")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      apollo setup           — full or quick (auto-detected)
      apollo setup model     — just model/provider
      apollo setup terminal  — just terminal backend
      apollo setup gateway   — just messaging platforms
      apollo setup tools     — just tool configuration
      apollo setup agent     — just agent settings
    """
    ensure_apollo_home()

    config = load_config()
    apollo_home = get_apollo_home()
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     Apollo Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    from apollo_cli.auth import get_active_provider
    ...
    print(
        color(
            "│             Apollo Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Apollo Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Apollo configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, apollo_home)
            return
        elif choice == 1:
            ...
            func(config)
            save_config(config)
            _print_setup_summary(config, apollo_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        if _offer_openclaw_migration(apollo_home):
    ...
    print_info(f"Data folder:  {apollo_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'apollo config edit'")
...
def _run_quick_setup(config: dict, apollo_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'apollo setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Apollo to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'apollo setup gateway'.")
