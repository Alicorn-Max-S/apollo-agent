"""
Configuration management for Apollo Agent.

Config files are stored in ~/.apollo/ for easy access:
- ~/.apollo/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.apollo/.env         - API keys and secrets

This module provides:
- apollo config          - Show current configuration
- apollo config edit     - Open config in editor
- apollo config set      - Set a specific value
- apollo config wizard   - Re-run setup wizard
"""
...
from apollo_cli.default_soul import DEFAULT_SOUL_MD
...
def get_apollo_home() -> Path:
    """Get the Apollo home directory (~/.apollo)."""
    return Path(os.getenv("APOLLO_HOME", Path.home() / ".apollo"))

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_apollo_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_apollo_home() / ".env"
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into APOLLO_HOME if the user doesn't have one yet."""
...
def ensure_apollo_home():
    """Ensure ~/.apollo directory structure exists with secure permissions."""
...
DEFAULT_CONFIG = {
    ...
    # This section is only needed for apollo-specific overrides; everything else
    ...
    "quick_commands": {},
    # Personality modes — switch with /personality <name>
...
OPTIONAL_ENV_VARS = {
    ...
    # APOLLO_TOOL_PROGRESS and APOLLO_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "APOLLO_TOOL_PROGRESS": {
    ...
    "APOLLO_EPHEMERAL_SYSTEM_PROMPT": {
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("APOLLO_TOOL_PROGRESS")
            old_mode = get_env_value("APOLLO_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from APOLLO_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from APOLLO_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("APOLLO_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from APOLLO_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: apollo config set <key> <value>")
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.apollo/config.yaml."""
    import copy
    ensure_apollo_home()
...
#   openai-codex (OAuth — apollo login) — OpenAI Codex
#   nous         (OAuth — apollo login) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.apollo/config.yaml."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.apollo/.env."""
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.apollo/.env."""
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.apollo/.env or environment."""
...
def show_config():
    ...
    print(color("│              Apollo Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  apollo config edit     # Edit config file", Colors.DIM))
    print(color("  apollo config set <key> <value>", Colors.DIM))
    print(color("  apollo setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or not value:
            print("Usage: apollo config set <key> <value>")
            print()
            print("Examples:")
            print("  apollo config set model anthropic/claude-sonnet-4")
            print("  apollo config set terminal.backend docker")
            print("  apollo config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'apollo config migrate' to add them")
        ...
        print("  apollo config           Show current configuration")
        print("  apollo config edit      Open config in editor")
        print("  apollo config set <key> <value>   Set a config value")
        print("  apollo config check     Check for missing/outdated config")
        print("  apollo config migrate   Update config with new options")
        print("  apollo config path      Show config file path")
        print("  apollo config env-path  Show .env file path")
        sys.exit(1)
