"""
Unified tool configuration for Apollo Agent.

`apollo tools` and `apollo setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.apollo/config.yaml under
...
# They're still in _APOLLO_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "rl", "image_gen", "tts"}

# Platform display config
PLATFORMS = {
    "cli":      {"label": "🖥️  CLI",       "default_toolset": "apollo-school"},
    "telegram": {"label": "📱 Telegram",   "default_toolset": "apollo-telegram"},
    "discord":  {"label": "💬 Discord",    "default_toolset": "apollo-discord"},
    "slack":    {"label": "💼 Slack",      "default_toolset": "apollo-slack"},
    "whatsapp": {"label": "📱 WhatsApp",   "default_toolset": "apollo-whatsapp"},
    "signal":   {"label": "📡 Signal",     "default_toolset": "apollo-signal"},
    "email":    {"label": "📧 Email",      "default_toolset": "apollo-email"},
}
    ...
    if post_setup_key == "browserbase":
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning("    npm install failed - run manually: cd ~/.apollo/apollo-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in apollo-agent directory)")
...
def _prompt_toolset_checklist(platform_label: str, enabled: Set[str]) -> Set[str]:
    ...
    from apollo_cli.curses_ui import curses_checklist
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `apollo tools` and `apollo setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("Apollo Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print()
    print(color("  Tool configuration saved to ~/.apollo/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'apollo' or gateway restart.", Colors.DIM))
    print()
