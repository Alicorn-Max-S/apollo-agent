"""
Gateway subcommand for apollo CLI.

Handles: apollo gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from apollo_cli.config import get_env_value, get_apollo_home, save_env_value
...
SERVICE_DESCRIPTION = "Apollo Agent Gateway - Messaging Platform Integration"
...
def get_launchd_plist_path() -> Path:
    return Path.home() / "Library" / "LaunchAgents" / "ai.apollo.gateway.plist"
...
def get_apollo_cli_path() -> str:
    """Get the path to the apollo CLI."""
    ...
    return f"{get_python_path()} -m apollo_cli.main"
...
def generate_systemd_unit() -> str:
    ...
    apollo_cli = shutil.which("apollo") or f"{python_path} -m apollo_cli.main"
    return f"""[Unit]
...
[Service]
Type=simple
ExecStart={python_path} -m apollo_cli.main gateway run --replace
ExecStop={apollo_cli} gateway stop
...
def refresh_systemd_unit_if_needed() -> bool:
    ...
    print("↻ Updated gateway service definition to match the current Apollo install")
    return True
...
def systemd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print(f"  apollo gateway start              # Start the service")
    print(f"  apollo gateway status             # Check status")
...
def systemd_status(deep: bool = False):
    ...
    if not unit_path.exists():
        ...
        print("  Run: apollo gateway install")
        return

    if not systemd_unit_is_current():
        print("⚠ Installed gateway service definition is outdated")
        print("  Run: apollo gateway restart  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print("  Run: apollo gateway start")
...
<dict>
    <key>Label</key>
    <string>ai.apollo.gateway</string>
    
    <key>ProgramArguments</key>
    <array>
        <string>{python_path}</string>
        <string>-m</string>
        <string>apollo_cli.main</string>
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  apollo gateway status             # Check status")
    print("  tail -f ~/.apollo/logs/gateway.log  # View logs")
...
def launchd_start():
    subprocess.run(["launchctl", "start", "ai.apollo.gateway"], check=True)
    print("✓ Service started")

def launchd_stop():
    subprocess.run(["launchctl", "stop", "ai.apollo.gateway"], check=True)
    print("✓ Service stopped")
...
def launchd_status(deep: bool = False):
    result = subprocess.run(
        ["launchctl", "list", "ai.apollo.gateway"],
        capture_output=True,
        text=True
    )
    ...
    if deep:
        log_file = get_apollo_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Apollo agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Apollo will use (e.g., apollo@gmail.com)."},
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_apollo_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'apollo pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: apollo pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'apollo gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from apollo_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"ApolloAgent\"")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: apollo gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: apollo gateway")
    else:
        print()
        print_info("No platforms configured. Run 'apollo gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: apollo gateway run")
            sys.exit(1)
        ...
        if is_linux() and get_systemd_unit_path().exists():
            ...
            if pids:
                ...
                print()
                print("To start:")
                print("  apollo gateway          # Run in foreground")
                print("  apollo gateway install  # Install as service")
