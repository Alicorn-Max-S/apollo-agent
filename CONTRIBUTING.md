...
Thank you for contributing to Apollo Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want Apollo to work everywhere.
...
Bundled skills (in `skills/`) ship with every Apollo install. They should be **broadly useful to most users**:
...
```bash
mkdir -p ~/.apollo/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.apollo/config.yaml
touch ~/.apollo/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.apollo/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/apollo" ~/.local/bin/apollo
...
├── cli.py                    # ApolloCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (apollo-cli, apollo-telegram, etc.)
├── apollo_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── apollo_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.apollo/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (apollo-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.apollo/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.apollo/`)

| Path | Purpose |
|------|---------|
| `~/.apollo/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.apollo/.env` | API keys and secrets |
| `~/.apollo/auth.json` | OAuth credentials (Nous Portal) |
| `~/.apollo/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.apollo/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.apollo/state.db` | SQLite session database |
| `~/.apollo/sessions/` | JSON session logs |
| `~/.apollo/cron/` | Scheduled job data |
| `~/.apollo/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`apollo_state.py`) with full-text search and unique session titles. JSON logs go to `~/.apollo/sessions/`.
...
Four fields are supported under `metadata.apollo`:
...
The user may skip setup and keep loading the skill. Apollo only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `apollo setup` or update `~/.apollo/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Apollo tools (`webscrape`, `terminal`, `read_file`).
...
- **Test it.** Run `apollo --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Apollo uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.apollo/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `apollo_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `apollo_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Apollo runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
Apollo has terminal access. Security matters.
...
2. **Test manually**: Run `apollo` and exercise the code path you changed
