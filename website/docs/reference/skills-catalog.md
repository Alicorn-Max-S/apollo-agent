...
| `websearch` | Student-focused web search using the built-in web_search tool (Firecrawl). Results are safe-search filtered, prioritised from trusted educational sources, and include APA citations. Use webscraping skill to analyze full page content from results. | `research/websearch` |
