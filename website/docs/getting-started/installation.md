...
sidebar_position: 2
title: "Installation"
description: "Install Apollo Agent on Linux, macOS, or WSL2"
---

# Installation

Get Apollo Agent up and running in under two minutes with the one-line installer, or follow the manual steps for full control.
...
:::warning Windows
Native Windows is **not supported**. Please install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Apollo Agent from there. The install command above works inside WSL2.
:::

### What the Installer Does

The installer handles everything automatically — all dependencies (Python, Node.js, ripgrep, ffmpeg), the repo clone, virtual environment, global `apollo` command setup, and LLM provider configuration. By the end, you're ready to chat.
...
apollo gateway setup  # Set up messaging platforms
apollo config set     # Set individual config values
apollo setup          # Or run the full setup wizard to configure everything at once
```
...
:::tip
You do **not** need to activate the venv to use `apollo`. The entry point has a hardcoded shebang pointing to the venv Python, so it works globally once symlinked.
:::
...
mkdir -p ~/.apollo/{cron,sessions,logs,memories,skills,pairing,hooks,image_cache,audio_cache,whatsapp/session}
...
Open `~/.apollo/.env` and add at minimum an LLM provider key:
...
```bash
apollo config set OPENROUTER_API_KEY sk-or-v1-your-key-here
```

### Step 8: Add `apollo` to Your PATH
...
```bash
apollo model       # Select your LLM provider and model
```

### Step 10: Verify the Installation

```bash
apollo version    # Check that the command is available
apollo doctor     # Run diagnostics to verify everything is working
apollo status     # Check your configuration
apollo chat -q "Hello! What tools do you have available?"
```
...
# Make apollo available globally
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/apollo" ~/.local/bin/apollo
...
| Problem | Solution |
|---------|----------|
| `apollo: command not found` | Reload your shell (`source ~/.bashrc`) or check PATH |
| `API key not set` | Run `apollo model` to configure your provider, or `apollo config set OPENROUTER_API_KEY your_key` |
| Missing config after update | Run `apollo config check` then `apollo config migrate` |

For more diagnostics, run `apollo doctor` — it will tell you exactly what's missing and how to fix it.
