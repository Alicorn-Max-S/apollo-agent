...
metadata:
  apollo:
    tags: [Migration, OpenClaw, Apollo, Memory, Persona, Import]
    related_skills: [apollo-agent]
---

# OpenClaw -> Apollo Migration

Use this skill when a user wants to move their OpenClaw setup into Apollo Agent with minimal manual cleanup.
...
```bash
apollo claw migrate              # Full interactive migration
apollo claw migrate --dry-run    # Preview what would be migrated
apollo claw migrate --preset user-data   # Migrate without secrets
apollo claw migrate --overwrite  # Overwrite existing conflicts
apollo claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `apollo setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_apollo.py` to:

- import `SOUL.md` into the Apollo home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Apollo memory entries
- merge OpenClaw command approval patterns into Apollo `command_allowlist`
- migrate Apollo-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.apollo/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Apollo workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.apollo/tts/`
- archive non-secret docs that do not have a direct Apollo destination
...
- `scripts/openclaw_to_apollo.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.apollo/skills/migration/openclaw-migration/scripts/openclaw_to_apollo.py`

Do not guess a shorter path like `~/.apollo/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.apollo/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Apollo-compatible secrets, currently:
...
Apollo CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Apollo. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Apollo workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Apollo skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.apollo/skills/migration/openclaw-migration/scripts/openclaw_to_apollo.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.apollo/skills/migration/openclaw-migration/scripts/openclaw_to_apollo.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.apollo/skills/migration/openclaw-migration/scripts/openclaw_to_apollo.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.apollo/skills/migration/openclaw-migration/scripts/openclaw_to_apollo.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.apollo/skills/migration/openclaw-migration/scripts/openclaw_to_apollo.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.apollo/skills/migration/openclaw-migration/scripts/openclaw_to_apollo.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Apollo unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Apollo targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Apollo destination. Unsupported auth blobs must still be reported as skipped.
...
- Apollo persona state imported
- Apollo memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.apollo/skills/openclaw-imports/`
